...
> macOS 菜单栏里的 AI Token 余额仪表盘，先照顾 Kimi Code / Zhipu Coding / MiniMax 这类有滚动额度的 Coding 套餐。
...
- 除 Kimi Code / Zhipu Coding / DeepSeek / MiniMax 外，没有通用网页自动登录采集器。
- 多设备同步、云端存储、团队共享面板。
...
- 上游网页和内部接口可能变化，Kimi/Zhipu/DeepSeek/MiniMax 适配器属于 best effort，失效时欢迎提 issue 或 PR。
